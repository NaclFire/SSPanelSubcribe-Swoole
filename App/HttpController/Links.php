...
//        }
        ...
        // 当前访问 URL
        if (isset($this->request()->getServerParams()['query_string'])) {
            $Rule['URL'] = $config->getConf('baseUrl') . $this->request()->getServerParams()['request_uri'] . '?' . $this->request()->getServerParams()['query_string'];
        } else {
            $Rule['URL'] = $config->getConf('baseUrl') . $this->request()->getServerParams()['request_uri'];
        }
        ...
        if (!$hasSubType) {
            ...
            $SubscribeExtend = $this->getSubscribeExtend($client);
            $filename = $SubscribeExtend['filename'] . '_' . time() . '.' . $SubscribeExtend['suffix'];
            $subscribe_type = $SubscribeExtend['filename'];
//            $content = Subcribe::getCommonSub($user, $Rule);
            $class = ('get' . $SubscribeExtend['class']);
            $content = Subcribe::$class($user, 0, $opts, $Rule);
        }
